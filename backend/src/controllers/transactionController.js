...
/**
 * GET /api/transactions/loan-summary
 * Returns existing transaction totals and average monthly estimates.
 */
const getLoanSummary = async (req, res) => {
  try {
    ...
    const summary = await transactionService.getLoanSummary(req.userId);
    return res.status(200).json({ success: true, ...summary });
  } catch (error) {
    console.error("Get loan summary error:", error);
    return res.status(500).json({ success: false, message: "Unable to retrieve financial data" });
  }
};
