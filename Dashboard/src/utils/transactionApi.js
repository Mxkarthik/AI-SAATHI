...
export const getLoanSummary = () => request("/api/transactions/loan-summary");

export const uploadLoanDocumentForOcr = (documentType, file) => {
  const formData = new FormData();
  formData.append("documentType", documentType);
  formData.append("document", file);

  return fetch("/api/loan-documents/ocr", {
    method: "POST",
    credentials: "include",
    body: formData,
  }).then(async (res) => {
    const data = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(data.message || "Unable to process this document.");
    return data;
  });
};
