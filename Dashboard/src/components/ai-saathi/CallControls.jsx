...
export default function CallControls({ isMuted, voiceStatus, recorderStatus = "idle", voiceState = "idle", disabled = false, onToggleMute, onStartVoice, onStopVoice, onLeave, onSettings }) {
  const { t } = useLanguage();
  ...
  const microphoneDisabled = disabled || voiceConnecting || transcribing;
  const microphoneLabel = voiceState === "listening"
    ? t("schemeAI", "listening")
    : voiceState === "transcribing"
      ? t("schemeAI", "transcribing")
      : voiceState === "thinking"
        ? t("schemeAI", "thinking")
        : voiceState === "speaking"
          ? t("schemeAI", "speaking")
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-3" aria-label={t("schemeAI", "callControls")}>
      <button
        ...
        aria-label={recording ? t("schemeAI", "stopRecording") : microphoneLabel}
